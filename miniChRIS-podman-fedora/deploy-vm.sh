...
sudo python3 profile_generator/profile_generator.py update_yaml miniChRIS-podman-fedora miniChRIS-podman-fedora/template.yaml --image ${IMAGE_NAME} --user fedora --user-password ${PASSWORD} --net-name ${NET_NAME}  --disk-size ${DISK_SIZE} 
cat  kcli-profiles.yml
ansiblesafe -f "${ANSIBLE_VAULT_FILE}" -o 1
sudo cp kcli-profiles.yml ~/.kcli/profiles.yml
sudo cp kcli-profiles.yml /root/.kcli/profiles.yml
echo "Creating VM ${VM_NAME}"
sudo kcli create vm -p miniChRIS-podman-fedora ${VM_NAME} --wait